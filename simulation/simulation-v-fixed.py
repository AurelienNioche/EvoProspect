...
data["vmin"] = data["vmax"] = 0.0
parameters.dump(data)
locals().update(data)
np.random.seed(seed)

recompute = False # ! Force recompute or not
dat_filename_1 = "data/v-fixed/simulation-initial-population"
dat_filename_2 = "data/v-fixed/simulation-final-population"
prm_filename = "data/v-fixed/simulation-parameters.json"
fig_filename = "figs/simulation-v-fixed-results.pdf"
...
if recompute:
    ...
    for i in range(n_agent):
        ...
        agent["v"][i] = V_distortion(lottery["v"], Av)
    score = play(lottery, agent, n_trial)    
    np.savetxt("initial-v-linear-p-variable.csv", score, delimiter=",")
    ...
    for i in range(n_agent):
        ...
        agent["v"][i] = V_distortion(lottery["v"], Av)
    score = play(lottery, agent, n_trial)
    np.savetxt("final-v-linear-p-variable.csv", score, delimiter=",")
...
plot(axes[0], param_i[:,0], "Initial population", "probability")
plot(axes[1], param_f[:,0],
     "Final population", "probability", display_mean=True)

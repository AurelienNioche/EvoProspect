...
fig_filename = "figs/simulation-histogram.png"
...
def plot(ax, P, title, xlabel, ylabel, idx_letter=None, display_mean=False):
    ...
    ax.hist(P, bins=20, edgecolor="white", linewidth=1)
    ...
    ax.text(0.5, 1.0, title, weight="bold",
...
plot(axes[0, 1], param_i[:, 1], "Initial population", r"$\beta$", "$n$",
     idx_letter=1)
plot(axes[1, 0], param_f[:, 0], "Final population", r"$\alpha$", "$n$",
     idx_letter=2,
     display_mean=True)
plot(axes[1, 1], param_f[:, 1], "Final population", r"$\beta$", "$n$",
     idx_letter=3,
     display_mean=True)
